		...
		<div class="m-right">
			...
			<div class="main">
				<div align="center" style=" margin:20px; line-height:28px;">
					<s:form action="admin_show!update.action" method="post"
						namespace="/">
						<s:hidden name="adminAuthority"
							value="%{#session.adminInfo.adminAuthority}"></s:hidden>
						<s:hidden name="adminInfo.adminId"
							value="%{#session.adminInfo.adminId}" />
						<s:hidden name="oldAdminAccount"
						    value="%{#session.adminInfo.adminAccount}"/>
						<s:textfield name="adminInfo.adminAccount"
							value="%{#session.adminInfo.adminAccount}" label="管理员账号" />
						<s:textfield name="adminInfo.adminName"
							value="%{#session.adminInfo.adminName}" label="管理员姓名" />
						<s:password name="adminInfo.adminPassword"
							value="%{#session.adminInfo.adminPassword}" label="管理员密码" />
						<s:select name="adminInfo.adminAuthority"
							list="#{0:'系统管理员', 1:'商品管理员', 2:'订单管理员', 3:'配送管理员'}"
							listKey="key" listValue="value"
							value="%{#session.adminInfo.adminAuthority}" label="权限" />
